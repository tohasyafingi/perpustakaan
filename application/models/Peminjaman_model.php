...
class Peminjaman_model extends CI_Model {
    ...
    // Ambil ID_PINJAM terakhir
    $query = $this->db->order_by('ID_PINJAM', 'DESC')->limit(1)->get('peminjaman')->row('ID_PINJAM');

    if ($query !== null) {
        $lastDate = substr($query, 1, 6); // Ambil tanggal dari ID
        $lastNumber = (int) substr($query, -3); // Ambil 3 digit terakhir sebagai nomor urut
    ...
    // Format akhir: P240525001 (misal)
    return 'P' . $today . sprintf('%03s', $nowID);
}
